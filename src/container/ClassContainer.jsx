...
export default function ClassContainer() {
  const [classesData, setClassesData] = useState(data.classesData);
  const [showFilterDate, setShowFilterDate] = useState(false);
  const [formData, setFormData] = useState({ filterDateValue: '' });
  const [showModal,setShowModal] = useState(false)

  const handleDelete = (index) => {
    const newclassesData = classesData.filter((_, i) => i !== index);
  ...
  return (
    <>
      <div className="mt-20 pb-4 bg-white dark:bg-gray-900 p-6 grid grid-cols-1">
        ...
        onClick={()=>setShowModal(true)}> New Class<FontAwesomeIcon icon={faPlus} className='ms-2' /></Button>
      </div>
      ...
      <Table datas={classesData} ></Table>
      <Modal showModal={showModal} setShowModal={setShowModal} title={'Enreigistrement de Class'}>
        <ClassForm/>
      </Modal>
    </>
  );
