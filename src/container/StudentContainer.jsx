...
export default function StudentContainer() {
  const [studentsData, setstudentsData] = useState(data.studentsData);
  const [showFilterDate, setShowFilterDate] = useState(false);
  const [formData, setFormData] = useState({ filterDateValue: '' });
  const [showModal,setShowModal] = useState(false)

  const handleDelete = (index) => {
    const newStudentData = studentsData.filter((_, i) => i !== index);
setstudentsData(newStudentData);
  ...
  return (
    <>
      ...
      <Table datas={studentsData} ></Table>
      <Modal showModal={showModal} setShowModal={setShowModal} title={"Enreigistrement D'elelve"}>
        <StudentForm/>
      </Modal>
    </>
  );
